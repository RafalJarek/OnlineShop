...
try
{
    ...
    $sql="SELECT u.id_user, u.login, pur.order_date, pro.name, purit.quantity, purit.price, pro.total_amount, s.status, pur.id_status, d.file_name
        ...
        inner join delivery as d on d.id_delivery = pur.id_delivery where s.id_status !='1' and s.id_status !='5' and s.id_status !='6' order by order_date DESC";
    ...
    foreach ($conn->query($sql) as $row) 
    {
        ...
        array_push($a_file_name_delivery,$row['file_name']);
    }
    $max=count($a_login);
}
...
//header
    echo '
        ...
        if($a_order_date[0]!=$a_order_date[1] && $a_id_status[0] != 5 && isset($first)) 
        {
            ...
            <div class='admin_change_status_dl'>
                <img src='$a_file_name_delivery[0]'></img>
            </div>

            <div class='admin_change_status_p'>
                <a href='../PHP/change_payment.php?login=".$a_id_user[0]."&date=".$a_order_date[0]."'><i class='fa fa-money fa-2x' aria-hidden='true'></i></a>
            </div>

            <div class='admin_change_status_d'>
                <a href='../PHP/change_delivery.php?login=".$a_id_user[0]."&date=".$a_order_date[0]."'><i class='fa fa-truck fa-2x' aria-hidden='true'></i></a>
            </div>
            
            <div class='admin_change_status_e'>
                <a href='../PHP/change_to_done.php?login=".$a_id_user[0]."&date=".$a_order_date[0]."'><i class='fa fa-check fa-2x' aria-hidden='true'></i></a>
            </div>
            ...
            echo '
                ...
                if(array_key_exists(($i+1),$a_order_date))
                {
                    ...
                    else
                    {
                        ...
                        <div class='admin_product_overview'>    
                            <div class='admin_change_status_dl'>
                                <img src='$a_file_name_delivery[$i]'></img>
                            </div>
                
                            <div class='admin_change_status_p'>
                                <a href='../PHP/change_payment.php?login=".$a_id_user[$i]."&date=".$a_order_date[$i]."'><i class='fa fa-money fa-2x' aria-hidden='true'></i></a>
                            </div>
                
                            <div class='admin_change_status_d'>
                                <a href='../PHP/change_delivery.php?login=".$a_id_user[$i]."&date=".$a_order_date[$i]."'><i class='fa fa-truck fa-2x' aria-hidden='true'></i></a>
                            </div>
                            
                            <div class='admin_change_status_e'>
                                <a href='../PHP/change_to_done.php?login=".$a_id_user[$i]."&date=".$a_order_date[$i]."'><i class='fa fa-check fa-2x' aria-hidden='true'></i></a>
                            </div>
